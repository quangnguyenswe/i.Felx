...
export class SnakeGame {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.superFoodImageReady = false;
    this.superFoodImage.src = "/images/db.svg";
    this.superFoodImage.addEventListener("load", () => {
      this.superFoodImageReady = true;
      this.redraw();
    });
    this.superFoodImage.addEventListener("error", () => {
      this.superFoodImageReady = false;
      this.redraw();
    });
    ...
    this.handleKeyDown = (event) => this.changeDirection(event);
  ...
  drawSuperFood() {
    if (this.superFood) {
      ...
      this.ctx.beginPath();
      this.ctx.arc(
        this.superFood.x * this.tileSize + this.tileSize / 2,
        this.superFood.y * this.tileSize + this.tileSize / 2,
        this.tileSize / 3,
        0,
        Math.PI * 2,
      );
  ...
  resetGame() {
    ...
    document
      .getElementById("right-image")
      ?.classList.remove("right-image-moved");
